...
@InternalApi private abstract class SingleSourceLogic[K, V, Msg](
  ...
  final def configureSubscription(): Unit = {
    ...
    def rebalanceListener(autoSubscription: AutoSubscription): KafkaConsumerActor.ListenerCallbacks = {
      val assignedCB = getAsyncCallback[Set[TopicPartition]](partitionAssignedCB)

      val revokedCB = getAsyncCallback[Set[TopicPartition]](partitionRevokedCB)
  ...
  protected def partitionRevokedCB(revokedTps: Set[TopicPartition]): Unit = {
    tps --= revokedTps
