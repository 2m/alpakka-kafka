...
private object TransactionalProducerStage {
  ...
  final class NonemptyTransactionBatch(head: PartitionOffsetCommittedMarker,
    ...
    def committedMarker: CommittedMarker = head.committer
    ...
    override def updated(partitionOffset: PartitionOffsetCommittedMarker): TransactionBatch = {
      ...
      require(
        this.committedMarker == partitionOffset.committer,
...
private final class TransactionalProducerStageLogic[K, V, P](stage: TransactionalProducerStage[K, V, P],
  ...
  override val onMessageAckCb: AsyncCallback[Envelope[K, V, P]] =
    getAsyncCallback[Envelope[K, V, P]](_.passThrough match {
      case o: ConsumerMessage.PartitionOffsetCommittedMarker =>
        ...
        println(s"ADDING OFF ${o.offset}")
      case x => println("Chuj " + x)
    })
  ...
  private def commitTransaction(batch: NonemptyTransactionBatch, beginNewTransaction: Boolean): Unit = {
    ...
    log.info("Committing transaction for consumer group '{}' with offsets: {}", group, batch.offsetMap())
    val offsetMap = batch.offsetMap()
    producer.sendOffsetsToTransaction(offsetMap.asJava, group)
    producer.commitTransaction()
    log.info("Committed transaction for consumer group '{}' with offsets: {}", group, batch.offsetMap())
    suspendDemand()
    batchOffsets = TransactionBatch.empty
    batch
      .internalCommit()
      .onComplete { _ =>
        onInternallCommitAckCb.invoke(beginNewTransaction)
      }(materializer.executionContext)
  }

  val onInternallCommitAckCb: AsyncCallback[Boolean] = {
    getAsyncCallback[Boolean](
      beginNewTransaction =>
